...
import {hasOpenHighE,midiNotes,voicingsFor,type GuitarRoute,type GuitarVoicing} from '../core/guitar/voicings'
...
type ProgressionState={chords:string[];voicingIds:(string|null)[]}
...
export function App(){
 const initialState:ProgressionState={chords:initialChords,voicingIds:initialChords.map(c=>voicingsFor(c)[0]?.id??null)}
 const [history,setHistory]=useState<History>({past:[],present:initialState,future:[]})
 const {chords,voicingIds}=history.present
 const commit=(next:ProgressionState)=>setHistory(h=>({past:[...h.past,h.present].slice(-40),present:next,future:[]}))
 const selectedVoicings=chords.map((chord,index)=>voicingsFor(chord).find(v=>v.id===voicingIds[index])??voicingsFor(chord)[0])
 ...
 const addChord=()=>{const parsed=parseChord(draft);if(!parsed){setError('Try a common symbol such as C, F#m, Bb, E7, Am9, or E/G#.');return}commit({chords:[...chords,parsed.normalized],voicingIds:[...voicingIds,voicingsFor(parsed.normalized)[0]?.id??null]});setDraft('');setError('')}
 const updateChord=(index:number,value:string)=>{const parsed=parseChord(value);if(!parsed){setError(`“${value}” is not a chord format this version knows yet.`);return}const next=[...chords],ids=[...voicingIds];next[index]=parsed.normalized;ids[index]=voicingsFor(parsed.normalized)[0]?.id??null;commit({chords:next,voicingIds:ids});setExpandedChord(null);setError('')}
 const move=(index:number,by:number)=>{const next=[...chords],ids=[...voicingIds],target=index+by;if(target<0||target>=next.length)return;[next[index],next[target]]=[next[target]!,next[index]!];[ids[index],ids[target]]=[ids[target]!,ids[index]!];commit({chords:next,voicingIds:ids});setExpandedChord(open=>open===index?target:open===target?index:open)}
 const selectVoicing=(index:number,id:string)=>{const ids=[...voicingIds];ids[index]=id;commit({chords:[...chords],voicingIds:ids})}
 const duplicateChord=(index:number)=>commit({chords:[...chords.slice(0,index+1),chords[index]!,...chords.slice(index+1)],voicingIds:[...voicingIds.slice(0,index+1),voicingIds[index]??null,...voicingIds.slice(index+1)]})
 const removeChord=(index:number)=>{commit({chords:chords.filter((_,i)=>i!==index),voicingIds:voicingIds.filter((_,i)=>i!==index)});setExpandedChord(null)}
 ...
 const addPath=(s:Suggestion,route?:GuitarRoute)=>{commit({chords:[...chords,...s.path],voicingIds:[...voicingIds,...(route?.voicings.map(v=>v.id)??s.path.map(c=>voicingsFor(c)[0]?.id??null))]});status(`Added ${s.path.join(' → ')}${route?' with your guitar path':''}`)}
 ...
 const save=()=>{const item:SavedProgression={id:crypto.randomUUID(),name:saveName.trim()||'Untitled idea',chords,voicingIds,tempo,updatedAt:new Date().toISOString()};const next=[item,...saves];setSaves(next);browserStorage.save(next);status('Progression and guitar shapes saved on this device')}
