...
export function App(){
 ...
 const addChord=(value:string)=>{const parsed=parseChord(value);if(!parsed)return;commit({chords:[...chords,parsed.normalized],voicingIds:[...voicingIds,voicingsFor(parsed.normalized)[0]?.id??null]});setEditingChord(null)}
 const updateChord=(index:number,value:string)=>{const parsed=parseChord(value);if(!parsed)return;const next=[...chords],ids=[...voicingIds];next[index]=parsed.normalized;ids[index]=voicingsFor(parsed.normalized)[0]?.id??null;commit({chords:next,voicingIds:ids});setExpandedChord(null);setEditingChord(null)}
