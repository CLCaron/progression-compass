...
export function App(){
 ...
 const updateChord=(index:number,value:string)=>{const parsed=parseChord(value);if(!parsed){setError(`“${value}” is not a chord format this version knows yet.`);return}const next=[...chords],ids=[...voicingIds];next[index]=parsed.normalized;ids[index]=voicingsFor(parsed.normalized)[0]?.id??null;commit({chords:next,voicingIds:ids});setError('')}
 const move=(index:number,by:number)=>{const next=[...chords],ids=[...voicingIds],target=index+by;if(target<0||target>=next.length)return;[next[index],next[target]]=[next[target]!,next[index]!];[ids[index],ids[target]]=[ids[target]!,ids[index]!];commit({chords:next,voicingIds:ids})}
 ...
 const removeChord=(index:number)=>commit({chords:chords.filter((_,i)=>i!==index),voicingIds:voicingIds.filter((_,i)=>i!==index)})
